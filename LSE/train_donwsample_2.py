...
lr = 1e-3
dataset_folder = 'data/features_no_img' if '-d' not in sys.argv else float(sys.argv[sys.argv.index('-d')+1]) 
...
settings_dict = {'BATCH_SIZE': bs, 'N_EPOCHS': nepochs, 'LR': lr,   
    ...
    'DATASET' : dataset_folder, 'max_seq_len' : max_seq_len,
    }


with open(os.path.join(WORKING_DIR, 'settings.dict'), 'w', encoding = 'utf-8') as f:
...
list_of_data = [_deserialize_data(fn, exclude=[]) for fn in data_fns]
...
train_rhands = [d['mp_hands (R,L)'][0][np.linspace(0, d['mp_hands (R,L)'][0].shape[0]-1, max_seq_len, dtype = int), ::]
                for d in train_data]
train_rhands = list(map(zero_to_one, train_rhands))                            # MAKE NON DETECTED KP TO BE -1

test_rhands = [d['mp_hands (R,L)'][0][np.linspace(0, d['mp_hands (R,L)'][0].shape[0]-1, max_seq_len, dtype = int), ::]
               for d in test_data]
test_rhands = list(map(zero_to_one, test_rhands))                              # MAKE NON DETECTED KP TO BE -1

train_lhands = [d['mp_hands (R,L)'][1][np.linspace(0, d['mp_hands (R,L)'][0].shape[0]-1, max_seq_len, dtype = int), ::]
                for d in train_data]
train_lhands = list(map(zero_to_one, train_lhands))                            # MAKE NON DETECTED KP TO BE -1

test_lhands = [d['mp_hands (R,L)'][1][np.linspace(0, d['mp_hands (R,L)'][0].shape[0]-1, max_seq_len, dtype = int), ::]
               for d in test_data]
test_lhands = list(map(zero_to_one, test_lhands))                              # MAKE NON DETECTED KP TO BE -1

train_face = [d['mp_face'][np.linspace(0, d['mp_hands (R,L)'][0].shape[0]-1, max_seq_len, dtype = int), ::]
              for d in train_data]
train_face = list(map(zero_to_one, train_face))                                # MAKE NON DETECTED KP TO BE -1

test_face = [d['mp_face'][np.linspace(0, d['mp_hands (R,L)'][0].shape[0]-1, max_seq_len, dtype = int), ::]
             for d in test_data]
test_face = list(map(zero_to_one, test_face))                                  # MAKE NON DETECTED KP TO BE -1

train_pose = [d['mp_pose'][np.linspace(0, d['mp_hands (R,L)'][0].shape[0]-1, max_seq_len, dtype = int), ::]
              for d in train_data]
train_pose = list(map(zero_to_one, train_pose))                                # MAKE NON DETECTED KP TO BE -1

test_pose = [d['mp_pose'][np.linspace(0, d['mp_hands (R,L)'][0].shape[0]-1, max_seq_len, dtype = int), ::]
             for d in test_data]
...
test_aug_embeddings = []
train_embeddings = []
train_aug_embeddings = []

if any([not hand_processor.built, not lstm_model.built]):
    batch = [[train_rhands[i] for i in [0]],
             [train_lhands[i] for i in [0]]]
...
# CREATING BATCHES TO GENERATE TRAIN AND TEST EMBEDDINGS FOR EVALUATION
...
#%%
# AUGMENTATING TRAIN BATCH FOR EVALAUTION
aligned_seq_batch = list(zip(batch_train[0], batch_train[1]))
batch_train_aug = augment_sequences(aligned_seq_batch)
batch_train_aug = list(zip(*batch_train_aug))
#%%
...
for ep in range(1, nepochs+1):
    ...
    with tf.GradientTape() as tape:
        for ib in idx_batches:
            ...
            input_2 = tf.convert_to_tensor(batch)
            # TRAIN STEP
            step_wise_loss = train_step(input_1, input_2)
            total_loss += step_wise_loss
...
metrics = np.zeros(shape = (len(epochs_to_eval),3)) # K = 1; K = 2; K = 5
...
plt.xlabel('Epochs')
plt.grid(True)
plt.savefig(os.path.join(WORKING_DIR,"accs_2.png"), dpi = 300)

#%%


max_per_idx = np.argmax(metrics[:,0])

train_embs = tf.math.l2_normalize(train_embeddings[max_per_idx], axis = 1).numpy()
test_embs = tf.math.l2_normalize(test_embeddings[max_per_idx], axis = 1).numpy()
all_embs = np.concatenate([train_embs, test_embs], axis = 0)
labels = [s+str(i) for s in ['a', 'b'] for i in range(train_embs.shape[0])]

np.savetxt(os.path.join(WORKING_DIR,"best_embs.tsv"), all_embs, delimiter="\t")
np.savetxt(os.path.join(WORKING_DIR,"labels.tsv"), labels, delimiter="\t", fmt = "%s")
