...
lr = 1e-4
dataset_folder = "../data_SSL/videos"
...
report = np.zeros(shape = (nepochs, 5))
...
for ep in range(1, nepochs+1):
    ...
    for ib in idx_batches:
        ...
        # PAD
        max_batch_len = max([t.shape[0] for t in list(input_1[0])+list(input_2[0])])
        input_1 = tf.convert_to_tensor([[np.concatenate([m, np.zeros((max_batch_len-m.shape[0], m.shape[1],
                                          m.shape[2]))], 0) for m in b] for b in list(input_1)])
        input_2 = tf.convert_to_tensor([[np.concatenate([m, np.zeros((max_batch_len-m.shape[0], m.shape[1],
                                          m.shape[2]))], 0) for m in b] for b in list(input_2)])
...
epochs_to_eval = range(0,nepochs,5)
